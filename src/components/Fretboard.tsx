  ...
  return (
    <div className='flex flex-col mb-8'>
      <div className='mb-2 md:mb-4 text-center'>
        <button id='start_practice' type='button' disabled={loading || notePositions.length === 0} className={`${practice ? 'bg-red-700 hover:bg-red-500' : 'bg-emerald-700 hover:bg-emerald-500'} m-auto outline outline-2 outline-transparent p-2 rounded-sm shadow-equal-sm shadow-black text-slate-50 text-lg md:text-2xl transition-all ease-in-out duration-150 w-full md:w-96 active:scale-95 active:shadow-none disabled:bg-gray-500 disabled:cursor-not-allowed disabled:opacity-60  disabled:scale-100 disabled:shadow-none focus-visible:outline focus-visible:outline-black focus-visible:outline-4`} onClick={handlePractice}>{loading || notePositions.length === 0 ? 'Loading' : practice ? 'Stop Practice' : 'Start Practice'}</button>
      </div>
      <div className='flex flex-wrap gap-1 md:gap-6 justify-center my-2 md:my-4'>
        {intervals && intervals.map((interval, index) =>
          <div key={index} className={`${currentNote[0] === interval[0] && currentNote[1] === interval[1] && 'bg-teal-500 current-note outline-8 shadow-equal-md shadow-black'} bg-gray-200 flex flex-col group outline outline-2 outline-transparent p-2 rounded-full text-xl text-center transition-all ease-in-out duration-300`} id={`interval_${index}`}>
              ...
              if (checkNote([selectedFret.indexOf(fret), fretNote[1]])) {
                ...
                if (note) {
                  return <div className={`${currentNote[0] === note[0] && currentNote[1] === note[1] && 'current-note'} bg-gray-200 group h-8 relative rounded-sm text-center`} data-active-note='true' data-current-note={currentNote[0] === note[0] && currentNote[1] === note[1] && true} key={fret.indexOf(fretNote)}>
                    <span className='absolute left-[50%] top-[50%] bg-cyan-900 flex h-7 w-7 items-center justify-center origin-center outline outline-2 outline-transparent rounded-full scale-100 shadow-equal-sm shadow-gray-900 transition-all ease-in-out duration-300 translate-x-[-50%] translate-y-[-50%] group-[.current-note]:bg-teal-500 group-[.current-note]:outline-4 group-[.current-note]:scale-125 group-[.current-note]:shadow-equal-md'>
